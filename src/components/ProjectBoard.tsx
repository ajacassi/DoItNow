...
function ItemCard({ item, onOpenItem }: { item: ProjectItem; onOpenItem: (item: ProjectItem) => void }) {
  ...
  const body = (
    <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-3 transition hover:border-neutral-700">
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        {item.url && (
          <a href={item.url} target="_blank" rel="noreferrer" className="text-xs text-neutral-600 hover:text-neutral-400">
            ↗
          </a>
        )}
      </div>
...
export default function ProjectBoard({ project, onOpenItem, onNewIssueForStatus }: Props) {
  ...
  return (
    <div className="flex h-full gap-4 overflow-x-auto px-8 py-6">
      {Array.from(columns.entries()).map(([status, items]) => {
        ...
        return (
          <div key={status} className="flex w-72 shrink-0 flex-col rounded-xl bg-neutral-900/40">
